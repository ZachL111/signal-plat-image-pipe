...
`signal-plat-image-pipe` is centered on platform engineering work in Java. The technical target is: Package a Java local lab for image analysis with append-only fixtures, checkpoint recovery checks, and documented operating limits.
...
The score is deterministic. The current threshold is 157. Risk has a penalty of 7, latency has a penalty of 3, and weight contributes a bonus of 4.
