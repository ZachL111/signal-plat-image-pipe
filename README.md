...
signal-plat-image-pipe is a Java project for platform engineering. It focuses on this technical goal: Package a Java local lab for image analysis with append-only fixtures, checkpoint recovery checks, and documented operating limits.
...
The core module exposes a small scoring API. Inputs are simple numeric signals: demand, capacity, latency, risk, and weight. The score uses a threshold of 157, risk penalty 7, latency penalty 3, and weight bonus 4. Tests exercise the public API against the fixture cases in `fixtures/cases.csv`.
